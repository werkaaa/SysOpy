...
void close_connection() {
    if(server_alive==1) {
        send_message(server_sock_fd, LOGOUT, "");
    }
    ...
    while(move[0] == '0') {
        ...
        if(new_message != NULL) {
            if(new_message->type == PING) {
                ...
                send_message(server_sock_fd, PING, "");
            }
...
int main(int argc, char ** argv) {
    ...
    send_message(server_sock_fd, LOGIN_ATTEMPT, name);
        ...
        while(logged_in){
            ...
            if (new_message->type == PING){
                ...
                if(strcmp(new_message->message_data, "O")==0){
                    wait_for_input();
                    send_message(server_sock_fd, GAME_MOVE, move);
                    my_turn = 0;
                }
