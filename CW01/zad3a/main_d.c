...
int main(int argc, char **argv) {
    ...
    void * handle = dlopen("./libdiff.so", RTLD_LAZY);
    if(handle == NULL){
        printf("There was a problem with loading the library!");
        return 1;
    }

    main_array* (*create_main_array)(int);
    create_main_array = (main_array* (*)(int))dlsym(handle, "create_main_array") ;

    pair * (*create_pair_sequence)(char**, int);
    create_pair_sequence = (pair * (*)(char**, int))dlsym(handle, "create_pair_sequence") ;

    void (*print_pair_sequence)(pair *, int);
    print_pair_sequence = (void (*)(pair *, int))dlsym(handle, "print_pair_sequence");

    char * (*compare_pair)(pair *);
    compare_pair = (char * (*)(pair *))dlsym(handle, "compare_pair");

    int (*save_block_to_main_array)(main_array *, char *);
    save_block_to_main_array = (int (*)(main_array*, char* ))dlsym(handle, "save_block_to_main_array");

    void (*delete_block)(main_array *, int);
    delete_block = (void (*)(main_array *, int))dlsym(handle, "delete_block");

    void (*delete_operation)(block *, int);
    delete_operation = (void (*)(block *, int))dlsym(handle, "delete_operation");

    void (*free_main_array)(main_array *);
    free_main_array = (void (*)(main_array *))dlsym(handle, "free_main_array");
    ...
    time_report = fopen("results3a.txt", "a");
